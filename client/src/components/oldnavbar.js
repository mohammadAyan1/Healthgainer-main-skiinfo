...
import { Menu, X, Home, Box, Star, Info, Leaf, ShoppingCart, Phone } from 'lucide-react';
...
export default function Navbar() {
  ...
  return (
    <>
          ...
          <div className="flex items-center space-x-4 sm:space-x-6">
            ...
            >
              ...
              <span className="hidden sm:inline">
                {currentUser ? `${currentUser.firstName} ${currentUser.lastName}` : 'Account Log In'}
              </span>
            </div>
            ...
            <div className="flex items-center space-x-2 cursor-pointer hover:opacity-80">
              ...
              <span onClick={() => router.push('/faq')} className="hidden sm:inline">Ask Your Question</span>
            </div>
            <div className="flex items-center space-x-2 cursor-pointer hover:opacity-80">
              <div className="w-8 h-8 flex items-center justify-center bg-white rounded-full">
      ...
      {/* Navbar */}
      <nav
        className={`bg-white shadow-md fixed md:top-12 w-full z-40 transition-transform duration-300 ${visible ? 'translate-y-0' : '-translate-y-full'
          }`}
      >
        <div className="container mx-auto text-black flex justify-between items-center md:py-2 px-4 sm:px-6">
          ...
          <div className="hidden md:flex space-x-6 text-gray-600">
            <Link href="/" className={`flex items-center space-x-2 ${isActive('/') ? 'text-primary font-semibold' : 'text-gray-600 hover:text-primary'}`}>
              <Home size={18} />
              <span>Home</span>
            </Link>
            <Link href="/product" className={`flex items-center space-x-2 ${isActive('/product') ? 'text-primary font-semibold' : 'text-gray-600 hover:text-primary'}`}>
              <Box size={18} />
              <span>Product</span>
            </Link>
            <Link href="/testimonials" className={`flex items-center space-x-2 ${isActive('/testimonials') ? 'text-primary font-semibold' : 'text-gray-600 hover:text-primary'}`}>
              <Box size={18} />
              <span>Testimonials</span>
            </Link>
            <Link href="/mediareport" className={`flex items-center space-x-2 ${isActive('/mediareport') ? 'text-primary font-semibold' : 'text-gray-600 hover:text-primary'}`}>
              <Box size={18} />
            ...
            <Link href="/distributorform" className={`flex items-center space-x-2 ${isActive('/distributorform') ? 'text-primary font-semibold' : 'text-gray-600 hover:text-primary'}`}>
              <Leaf size={18} />
              <span>Get Distributorship</span>
            </Link>
            <Link href="/about" className={`flex items-center space-x-2 ${isActive('/about') ? 'text-primary font-semibold' : 'text-gray-600 hover:text-primary'}`}>
              <Info size={18} />
              <span>About</span>
            </Link>
            ...
            <Link href="/contact" className={`flex items-center space-x-2 ${isActive('/contact') ? 'text-primary font-semibold' : 'text-gray-600 hover:text-primary'}`}>
              <Phone size={18} />
              <span>Contact</span>
            </Link>
          </div>
        ...
        {isOpen && (
          <div className="md:hidden flex flex-col space-y-4 bg-white shadow-md p-4 text-gray-800">
            ...
            <div className="flex items-center justify-between border-b pb-3">
              <div onClick={handleProfileClick} className="flex items-center space-x-3">
                {user ? (
                  <img src={user?.profilePhoto || "/default-avatar.png"} alt="Profile" className="w-10 h-10 rounded-full border" />
                ) : (
                  <MdAccountCircle className="text-2xl text-black" />
                )}
            ...
            <div className="flex items-center space-x-2 cursor-pointer hover:opacity-80">
              ...
              <span onClick={() => router.push('/faq')} className=" sm:inline">Ask Your Question</span>
            </div>
